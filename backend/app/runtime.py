...
from .ports import default_output_port, project_node_output
...
def _make_node_runner(node, ctx: ExecContext):
    ...
    async def run_node(state: RunState) -> dict[str, Any]:
        ...
        delta = dict(delta)
        # P0 graph foundation, Slice A: project onto the node's declared
        # output port(s) instead of writing the raw executor value directly.
        # `{}` for resolved_inputs — Slice A's projection doesn't read it;
        # Slice B populates it for router/branch's passthrough/decision split.
        delta["node_outputs"] = {
            **delta.get("node_outputs", {}),
            node.id: project_node_output(node, {}, output),
        }
        _merge_into_snapshot(ctx.state_snapshot, delta)
        return delta
