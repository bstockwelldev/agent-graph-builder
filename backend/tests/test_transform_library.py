...
FACT_LINE = {"id": "fact_line", "name": "Fact line", "type": "format_message", "template": "Fact: {value}"}
...
async def _run(graph, **kwargs):
    ...
    run_id, _bus = await start_run_inline(
        f"cwf_{graph.id}", {"question": "How does a database index work?"}, provider="stub", **kwargs
    )
...
def test_missing_reference_is_an_edge_scoped_error() -> None:
    diagnostics = [
        d for d in validate_graph(_graph_with_ref(ref="gone")) if d.code == "UNRESOLVED_RESOURCE_BINDING"
    ]
...
def test_unresolved_reference_blocks_release() -> None:
    ...
    assert [(d.code, d.edge_id) for d in diagnostics] == [("RELEASE_RESOURCE_UNRESOLVED", "e_tool_output")]
