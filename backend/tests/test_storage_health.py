...
def test_storage_is_healthy_on_vercel_with_blob_token(monkeypatch: pytest.MonkeyPatch) -> None:
    _clear_durable_env(monkeypatch)
    monkeypatch.setenv("VERCEL", "1")
...
def test_health_reports_healthy_on_vercel_with_blob(monkeypatch: pytest.MonkeyPatch) -> None:
    _clear_durable_env(monkeypatch)
    monkeypatch.setenv("VERCEL", "1")
    monkeypatch.setenv("BLOB_READ_WRITE_TOKEN", "vercel_blob_rw_teststore_testtoken")
    monkeypatch.setattr(storage.vercel_blob, "get_graph", lambda graph_id: build_demo_graph())
    monkeypatch.setattr(storage.vercel_blob, "save_graph", lambda graph: None)
...
def test_api_routes_work_on_vercel_with_blob_token(monkeypatch: pytest.MonkeyPatch) -> None:
    _clear_durable_env(monkeypatch)
    monkeypatch.setenv("VERCEL", "1")
    monkeypatch.setenv("BLOB_READ_WRITE_TOKEN", "vercel_blob_rw_teststore_testtoken")
    monkeypatch.setattr(storage.vercel_blob, "list_graphs", lambda: [build_demo_graph()])
    monkeypatch.setattr(storage.vercel_blob, "get_graph", lambda graph_id: build_demo_graph())
    monkeypatch.setattr(storage.vercel_blob, "save_graph", lambda graph: None)
