...
async def test_guardrail_passes_clean_text() -> None:
    node = GraphNode(id="guard_1", type=NodeType.GUARDRAIL, position=NodePosition(x=0, y=0), config={})
...
async def test_guardrail_allows_url_when_configured() -> None:
    node = GraphNode(
        id="guard_1", type=NodeType.GUARDRAIL, position=NodePosition(x=0, y=0), config={"allowUrls": True}
    )
    graph = _graph_with_node_before_output(node)
    summary = await _run_non_technical(graph, question="See https://example.com for toppings")
    assert summary.status == "succeeded"
...
async def test_rubric_passes_through_findings_without_failing_by_default() -> None:
    node = GraphNode(id="rubric_1", type=NodeType.RUBRIC, position=NodePosition(x=0, y=0), config={})
    graph = _graph_with_node_before_output(node)
    summary = await _run_non_technical(graph)
    assert summary.status == "succeeded"
...
async def test_rubric_blocks_on_findings_when_configured() -> None:
    ...
    demo = build_demo_graph()
    nodes = [
        n if n.id != "prompt_answer" else n.model_copy(update={"config": {"template": "{unresolved_var}"}})
        for n in demo.nodes
    ]
    rubric_node = GraphNode(
        id="rubric_1", type=NodeType.RUBRIC, position=NodePosition(x=0, y=0), config={"rubricFailOnFindings": True}
    )
...
async def test_branch_matched_takes_conditional_edge() -> None:
    demo = build_demo_graph()
    branch = GraphNode(id="branch_1", type=NodeType.BRANCH, position=NodePosition(x=0, y=0), config={"content": "pizza"})
    matched_output = GraphNode(id="matched_output", type=NodeType.OUTPUT, position=NodePosition(x=0, y=0), config={})
    ...
    edges.append(
        GraphEdge(id="e_branch_matched", source="branch_1", target="matched_output", kind=EdgeKind.CONDITIONAL, condition="stub")
    )
    edges.append(GraphEdge(id="e_branch_default", source="branch_1", target="output_1", kind=EdgeKind.DEFAULT))
    ...
    run_id, _bus = await start_run_inline("cwf_branch", {"question": "pizza toppings?"}, provider="stub")
    from app.runtime import get_run_summary, get_run_node_traces
...
async def test_branch_no_match_falls_back_to_default() -> None:
    demo = build_demo_graph()
    branch = GraphNode(
        id="branch_1", type=NodeType.BRANCH, position=NodePosition(x=0, y=0), config={"content": "not-in-the-output"}
    )
    matched_output = GraphNode(id="matched_output", type=NodeType.OUTPUT, position=NodePosition(x=0, y=0), config={})
    edges = [e for e in demo.edges if e.id != "e_llmanswer_output"]
    edges.append(GraphEdge(id="e_llmanswer_branch", source="llm_answer", target="branch_1"))
    ...
    run_id, _bus = await start_run_inline("cwf_branch_default", {"question": "pizza toppings?"}, provider="stub")
    from app.runtime import get_run_summary, get_run_node_traces
...
async def test_tool_loop_calls_lookup_topic_then_returns_final_answer() -> None:
    ...
    graph = demo.model_copy(
        update={"id": "graph_tool_loop", "nodes": [input_node, loop_node, output_node], "edges": edges, "entry_node_id": "input_1"}
    )

    compiled = compile_graph(graph, "cwf_tool_loop")
    assert compiled.ok, compiled.diagnostics
    COMPILED_WORKFLOWS["cwf_tool_loop"] = graph
    run_id, _bus = await start_run_inline("cwf_tool_loop", {"question": "how does a database index work?"}, provider="stub")
    from app.runtime import get_run_summary, get_run_node_traces

    summary = get_run_summary(run_id)
    assert summary.status == "succeeded"
    trace = next(t for t in get_run_node_traces(run_id) if t.node_id == "loop_1")
    tool_calls = [entry for entry in trace.input["transcript"] if entry.get("toolCall") == "lookup_topic"]
...
async def test_tool_loop_falls_back_to_final_answer_for_non_technical_question() -> None:
    ...
    graph = demo.model_copy(
        update={"id": "graph_tool_loop_2", "nodes": [input_node, loop_node, output_node], "edges": edges, "entry_node_id": "input_1"}
    )

    compiled = compile_graph(graph, "cwf_tool_loop_2")
    assert compiled.ok, compiled.diagnostics
    COMPILED_WORKFLOWS["cwf_tool_loop_2"] = graph
    run_id, _bus = await start_run_inline("cwf_tool_loop_2", {"question": "what's a good pizza topping?"}, provider="stub")
    from app.runtime import get_run_summary
