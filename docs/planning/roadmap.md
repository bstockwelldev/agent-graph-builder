...
| **P0** | **Studio consolidation Phase 1 — schema** | High | High | Absorb micro-ui-agent-builder's 11-node vocabulary into `NodeType` with typed configs; per-type compiler diagnostics. See [studio-consolidation-plan.md](features/studio-consolidation-plan.md). |
| **P1** | **Studio consolidation Phase 2 — executors** | High | High | `guardrail`/`rubric`/`branch`/`tool_loop`/`code_exec`/`human_gate` node executors; `FlowDocument → GraphDefinition` migration. |
| **P1** | **Studio consolidation Phase 3 — registries** | High | High | Stored prompts/tools/mcp_servers/agents/llm_profiles + CRUD; retires the hard-coded `lookup_topic` tool. |
| **P1** | **Studio consolidation Phase 4 — studio app** | High | High | `apps/studio` (Next.js) ported alongside `apps/playground`; pnpm migration; parity gate before cutover. |
| **P2** | **Studio consolidation Phase 5 — hardening** | Medium | High | Telemetry (Langfuse), RAG, Supabase auth, analytics ported from micro-ui-agent-builder. |
