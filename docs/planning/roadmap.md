...
| **P1** | **Diagnostics as a navigation system** | High | Debug loop | Confirmed gap (2026-09-22 UX review pass against `apps/studio`): clicking a compile/validation warning does nothing beyond showing inline text today — `FlowCanvas.tsx` has no pan/zoom/select path from a diagnostic to its node/edge/port. Make each diagnostic resolve to a `graphObjectId` (node/edge/port/field), and clicking it should focus the canvas on that object, select it, and open the relevant `NodeInspector` tab. Extends the P0 validation/contracts work and the existing selection dock (Slice B) rather than adding new surfaces. |
...
| **P1** | **Historical run waterfall** | Medium | Debug loop | Confirmed gap: no timing/sequencing visualization exists anywhere in the repo — run history (`/runs/[graphId]` panel) is a sortable table only. Add a per-run timing view (start/duration/queue time per node, parallel vs. sequential spans, retries) that's bidirectionally linked to the canvas: clicking a bar focuses the node, selecting a node can scroll/highlight its bar. Depends on the P0 run-timeline work landing first; pairs with Historical replay (Slice C). |
...
| **P2** | **Chat: bind the scratchpad to graph/selection/run context** | Medium | Core UX | Confirmed gap: `ChatPanel.tsx` is a direct provider/model scratchpad (`client.sendChatMessage`) that bypasses the graph engine entirely — no selected-node, selected-run, or diagnostic context is ever injected, and it only preselects a session ID from the command palette. Give Chat read access to current workbench selection (active graph, selected node/edge, open run/diagnostic) so questions like "why is this node failing" don't require the user to supply IDs. Prerequisite for the two rows below. |
| **P2** | **Remote/graph flow invocation from Chat** | Medium | Differentiator | Confirmed gap: Chat can never trigger graph execution today. Let Chat resolve a flow + release/version + environment from a request, execute it, and stream per-node status back into the same chat thread, linked to the resulting run. Depends on the Chat context-binding row above and on releases (`p0-graph-foundation-design-plan.md`) for version/environment resolution. |
| **P2** | **Collapsed reasoning/tool-use disclosure in Chat** | Low | Polish | Confirmed gap: there's currently no tool-call/reasoning trace to disclose because the scratchpad bypasses the graph engine. Once Chat is graph-bound and/or can invoke flows (the two rows above), render tool calls and reasoning as collapsed `▸ Tool: name · duration · status` rows (expand for input/output) instead of raw blocks. `components/ai-elements/*` (already in the tree, from Vercel's AI Elements) is the natural place to add this. |
