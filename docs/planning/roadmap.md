...
| **P2** | **SDK hardening & 1.0** | Medium | High for every consumer (Studio, Playground, integrators) | **Planned 2026-09-24** — [sdk-hardening-plan.md](features/sdk-hardening-plan.md). Seven sequenced PRs: 1 transport + typed errors (High, **shipped** — STO-614); 2 run lifecycle + universal streaming (High, **shipped** — STO-615); 3 OpenAPI-generated types + drift check (**shipped** — STO-616); 4 namespaced API + request objects + cursor pagination (**shipped** — STO-617); 5 `/graph` + `/testing` entry points (**shipped** — STO-620); 6 `/react` hooks (**shipped** — STO-618); 7 docs and release to 1.0 (**shipped** — STO-619; 1.0.0 versioned, npm publish via the manual Release SDK workflow). Linear [STO-613](https://linear.app/stockwise-productions-prototypes/issue/STO-613) (STO-614–620); GitHub [#64](https://github.com/bstockwelldev/agent-graph-builder/issues/64) (#65–#71). |
...
| ~~**P3**~~ | ~~**Mobile bottom nav tray**~~ | Low | Medium for mobile users | **Shipped 2026-09-23** ([STO-607](https://linear.app/stockwise-productions-prototypes/issue/STO-607)). `components/navigation/mobile-tab-bar.tsx`: a shared bottom tab bar. Non-canvas pages below `md` get **Graphs · Resources · Analytics · Chat · More**; Chat opens the chat drawer, and More opens the nav sheet, replacing the top-bar hamburger. The canvas (below 1100px) gets a contextual tray from the same component: **Graphs · Add · Run · Chat · More**. More holds Focus mode, Workflow summary, Releases, Routing lab, Knowledge and Shortcuts. This replaces the ad hoc `isCompact` icon row. See [studio-graph-workbench-redesign-plan.md](features/studio-graph-workbench-redesign-plan.md#wave-5-mobile-tab-bar--shipped-sto-607). |
...
| ~~**P1**~~ | ~~**Semantic graph diffs and review workflow**~~ | High | Differentiator | **Shipped, confirmed 2026-09-22.** `fingerprint.py:219` `diff_graphs`, `releases.py:175` diff endpoint, `ReleasesPanel.tsx` compare UI, `test_release_diff.py`. **Named gap:** compares two releases only, not a release against an arbitrary in-canvas draft. Slice A of [p1-rollout-plan.md](features/p1-rollout-plan.md). **Gap closed 2026-09-23** ([STO-609](https://linear.app/stockwise-productions-prototypes/issue/STO-609)): Releases → "Diff vs draft" compares a release with the live canvas, unsaved edits included (`POST /api/graph-releases/{id}/compare-draft`). |
...
| ~~**P1**~~ | ~~**Historical replay and counterfactual debugging**~~ | High | Differentiator | **Shipped, confirmed 2026-09-22.** `replay.py:109`, `test_replay.py` — read-only replay of a `RunGraphSnapshot` with frozen tool outputs. **Named gap:** counterfactual modes (alternate model/router/retriever/node versions) aren't built — only frozen-replay of the original run exists. Slice C of [p1-rollout-plan.md](features/p1-rollout-plan.md). **Counterfactual modes shipped 2026-09-23** ([STO-609](https://linear.app/stockwise-productions-prototypes/issue/STO-609)). History → "Replay with changes…" can force a router or branch onto a different target, or swap an LLM node's provider/model. A swapped node runs live when its provider has a key, and falls back to the stub otherwise. Nodes downstream of a change recompute on the stub, or on the original provider when "Run affected nodes live" is on; all other nodes stay frozen. The result shows each node's mode and its original vs new output. See [counterfactual-replay.md](features/counterfactual-replay.md). |
...
| ~~**P1**~~ | ~~**Routing policy lab**~~ | High | Differentiator | **Shipped, confirmed 2026-09-22.** `routing_lab.py`, `RoutingLabPanel.tsx`, `test_routing_lab.py`. **Named gap:** dataset-driven comparison is graph-vs-graph only, not graph-vs-release. Slice D of [p1-rollout-plan.md](features/p1-rollout-plan.md). **Gap closed 2026-09-23** ([STO-609](https://linear.app/stockwise-productions-prototypes/issue/STO-609)): the lab can now compare against a published release (baseline, with its frozen resources; `latest` allowed) as well as against another graph. |
| ~~**P2**~~ | ~~**Cross-cutting policy overlays**~~ | High | Enterprise | **Configurable, shipped 2026-09-23** ([STO-608](https://linear.app/stockwise-productions-prototypes/issue/STO-608)). The rule catalog now has per-rule enforcement (off / warn / block publish / block) and parameters, resolved in the order default → workspace → graph override. There are compile and publish gates, and exceptions can be extended or revoked. Studio has a `/policies` page, a graph Policies panel, and waive-for-7/30/90-days. See [configurable-policies.md](features/configurable-policies.md). |
...
| ~~**P2**~~ | ~~**Remote/graph flow invocation from Chat**~~ | Medium | Differentiator | **Shipped 2026-09-23.** "Run a graph" picker + `/run <graph>[@latest|@rel] …` in Chat call the same `startRun`/`startReleaseRun` API as the Run panel; run cards stream node progress and link to View run / View flow. Release runs and free-text proposals require an explicit Confirm (user decision); drafts from explicit commands run at once. Full design spec + acceptance criteria: [studio-ux-gap-remediation-plan.md §4](features/studio-ux-gap-remediation-plan.md#4-remoteflow-invocation-from-chat). Linear: [STO-600](https://linear.app/stockwise-productions-prototypes/issue/STO-600). |
| ~~**P2**~~ | ~~**Collapsed reasoning/tool-use disclosure in Chat**~~ | Low | Polish | **Shipped 2026-09-23** with the row above: `components/ai-elements/tool-step.tsx` renders each run step as a collapsed one-line row expanding to height-capped, truncatable input/output JSON. Full design spec + acceptance criteria: [studio-ux-gap-remediation-plan.md §5](features/studio-ux-gap-remediation-plan.md#5-collapsed-reasoningtool-use-disclosure-in-chat). Linear: [STO-601](https://linear.app/stockwise-productions-prototypes/issue/STO-601). |
...
| ~~**P3**~~ | ~~**Large-graph complexity management**~~ | Medium | Strategic pillar — **Done** | **Designed 2026-09-24** in [large-graph-complexity-plan.md](features/large-graph-complexity-plan.md), in four waves: 7a understand, 7b groups, 7c graph-as-node subgraphs, 7d views/layers. **7a shipped** ([STO-610](https://linear.app/stockwise-productions-prototypes/issue/STO-610)): find on canvas (Ctrl/Cmd+F), a directional dependency view, a node blast-radius Impact tab, and a 0–100 graph health score with a linked breakdown. **7b shipped** ([STO-611](https://linear.app/stockwise-productions-prototypes/issue/STO-611)): display-only visual groups (Ctrl/Cmd+G), collapse to a card with rerouted/merged edges, rename/colour/ungroup with undo, and Find/Health auto-expand. **7c shipped** ([STO-612](https://linear.app/stockwise-productions-prototypes/issue/STO-612)): graph-as-node `subgraph` nodes. They run as nested runs with their own trace and "Open child run"; their inputs map through templates; a draft run uses the child's latest release; publishing freezes the exact child release; cycle and depth diagnostics are enforced; plus "Extract to graph" and "Used by". **7d shipped** ([STO-622](https://linear.app/stockwise-productions-prototypes/issue/STO-622)): display-only architecture layers and a View menu with Canvas, Overview (all groups collapsed), Layers (rank-ordered swimlanes with lane filters) and Heatmap (p95 latency, failure rate or executions, with a legend), plus a Manage layers dialog with auto-assign. **Pillar complete.** |
