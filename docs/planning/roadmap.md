...
| ~~**P2**~~ | ~~**Studio consolidation Phase 6 — cutover**~~ | High | High | **Mostly shipped.** `vercel.json` repointed at `apps/studio` (verified via live production deploy); `apps/playground` deleted (66 files; two still-useful tests ported to `apps/studio/lib/`); CI's separate `playground` job folded into `studio`'s; `README.md`/`AGENTS.md` rewritten for the studio-only repo, `docs/stitch-reference/` + GitHub issue/PR templates ported from micro-ui-agent-builder, and its two cross-repo-referenced docs (`mcp-multi-repo-rollout.md`, `micro-ui-agent-builder-future-ai-stack-prd.md`) copied into this repo's `docs/`. **One step remains, gated on explicit user confirmation:** archiving `micro-ui-agent-builder` itself (irreversible, affects a repo outside this one) — see [studio-consolidation-plan.md](features/studio-consolidation-plan.md)'s Phase 6 as-built notes. |
| ~~**P2**~~ | ~~**Studio consolidation Phase 7 — UX parity and polish**~~ | Medium | Medium | **Shipped 2026-09-17.** Post-cutover review against MUI's design intent and the old playground's UX: upgraded node card visuals, restored persistent docked panels + in-canvas graph switcher, fixed a skeleton/shimmer CSS bug, regrouped sidebar nav into 3 sections, added a right-click context menu (new scope, not a restoration). |
| ~~**P2**~~ | ~~**Studio consolidation Phase 8 — workbench, command palette, chat sessions**~~ | High | Medium | **Shipped 2026-09-18.** App-wide workbench (`WorkbenchProvider`, hotkeys + right-click, "two doors" per panel), `Cmd/Ctrl+K` command palette, backend multi-turn chat (`ChatModel.generate(history=...)` across all 6 adapters) + persisted `ChatSession` resource, matching SDK additions, and a minimal chat scratchpad UI. Scope set by user direction (direct-model scratchpad, persisted from day one), not a plan-doc gap. |
