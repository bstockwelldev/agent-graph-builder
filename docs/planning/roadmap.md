...
### Phase 11 — Graph-native workspace redesign (P1–P2) — Waves 1, 2 and 2.5 shipped
...
4. **Wave 3 — Motion + accessibility.** Drawers slide, menus originate from triggers, Escape closes panels + focus trap, `reducedMotion` threaded to every panel, focus rings across the graph kit. Linear: [STO-604](https://linear.app/stockwise-productions-prototypes/issue/STO-604).
5. **Wave 4 — Resource binding + inspector consolidation.** Node↔registry references (cross-language contract), reverse "used by" API, one resource inspector replacing five duplicated page shells. Linear: [STO-605](https://linear.app/stockwise-productions-prototypes/issue/STO-605).
