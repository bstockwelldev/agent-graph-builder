...
| **P1** | **Studio consolidation Phase 4 — studio app** | High | High | `apps/studio` (Next.js) ported alongside `apps/playground` in sub-phases 4a–4f; parity gate before cutover. 4a–4e shipped (pnpm migration, design system, shell/CRUD, graph editor with all 12 node types, run surface + GenUI). Only 4f (SDK Zod hardening + playground parity checkpoint) remains. |
