...
| **P2** | **Collapsed reasoning/tool-use disclosure in Chat** | Low | Polish | Confirmed gap: there's currently no tool-call/reasoning trace to disclose because the scratchpad bypasses the graph engine. Unblocked now that Chat context-binding has shipped; the remote-invocation row above is still a prerequisite. `components/ai-elements/*` (already in the tree, from Vercel's AI Elements) is the natural place to add this. Full design spec + acceptance criteria: [studio-ux-gap-remediation-plan.md §5](features/studio-ux-gap-remediation-plan.md#5-collapsed-reasoningtool-use-disclosure-in-chat). |
