...
- **Step 3 (done):** deleted `apps/playground` entirely (66 files). Checked each of the three tests the plan flagged as possibly-unique: `canvasFit.test.ts` and `observePanel.test.ts` tested logic (`canFitView`, `nextExclusiveOpenId`/`resolveEventLogEvents`/`formatRunResult`/`eventLogEmptyMessage`) that studio already has ported source for (`apps/studio/lib/canvasFit.ts`, `apps/studio/lib/observePanel.ts` — byte-identical to playground's aside from one import path) but no test coverage yet, so both test files were ported verbatim into `apps/studio/lib/`. `shellLayout.test.ts` was **not** ported — it tests `apps/playground/src/shellLayout.ts`'s 4-column compact/drawer layout logic, which 4d's plan explicitly excluded from the studio port (superseded by MUI's HUD/rail chrome handling responsiveness via Tailwind breakpoints) and confirmed to have zero equivalent anywhere in `apps/studio`. Also found and fixed two real pre-existing bugs while auditing every repo-wide reference to "playground" before deleting it: (1) `apps/studio/content/taxonomy.ts` and `apps/studio/lib/graphAuthoring.ts` had **user-facing help copy that literally said "this playground"** (e.g. "this playground is acyclic only") inside the *studio* app — reworded to drop the app-name reference entirely rather than say "studio", since the constraint itself doesn't need to name the app. (2) Root `package.json`'s scripts (`build`, `build:playground`, `dev:playground`, `test`, `test:studio`) and `.github/workflows/ci.yml`'s separate `playground` CI job both still referenced the now-deleted app; consolidated to canonical studio-only script names (`build`, `build:sdk`, `dev`, `test` — no more `:studio`/`:playground` suffixes needed with only one app) and removed the now-dead `playground` CI job, folding its coverage into the existing `studio` job (which already builds the SDK first). Left a few harmless historical/attribution comments referencing `apps/playground` as-is (`pnpm-workspace.yaml`'s React 18/19 coexistence note, `next.config.ts`'s "mirrors apps/playground/vite.config.ts" comment, `nodeDefaults.ts`'s "ported from apps/playground" comment) — checked whether the `pnpm-workspace.yaml` `packageExtensions` React-types workaround (added specifically for two coexisting React majors) is now dead weight now that only React 19 remains: `pnpm install` + full typecheck/lint/build stayed clean either way, so it's provably harmless left in place; not removed since proving it's *required* to stay absent would need a separate clean-room install, out of scope for this step.
- **Gate:** full verification clean after deletion — `pnpm -r typecheck`/`lint` (0 errors, same 4 pre-existing studio warnings), `pnpm run test` (sdk 24 + studio 30, up from 13 with the two ported tests), `pnpm run build` (sdk + studio, 15 routes), `cd backend && uv run pytest -q` (226, untouched — this step touched no Python).
- **Step 4 (done):** rewrote `README.md` and `AGENTS.md` for the studio-only repo (playground fully gone from both — routes, providers, CI, storage backends, workbench/chat all documented); updated `docs/planning/roadmap.md`'s Phase 6 row (and added the missing Phase 7/8 rows, which had shipped but were never recorded there) to "mostly shipped" pending the archive step below. Ported MUI's `.github/pull_request_template.md` + `.github/ISSUE_TEMPLATE/*` with AGB's nouns substituted (Flows→Graphs, `pnpm lint`→`pnpm -r lint`, `app/(studio)/`→`apps/studio/app/`, etc.) — these are live templates contributors actually fill out, so the substitution is real, not cosmetic. Ported `docs/stitch-reference/` **verbatim, deliberately not substituting nouns** — a documented deviation from this plan's literal "substituting AGB's nouns" instruction: inspecting the content showed every filename/screenId/projectId is a live Stitch API identifier and Google-hosted download URL, not prose: renaming "flow-builder.html" or the Stitch project id would sever traceability to the actual Stitch project without changing anything real. Left `scripts/stitch-download-screen.mjs` and the `@google/stitch-sdk` dependency in the (now-archived) `micro-ui-agent-builder` repo rather than porting them too — this material is a frozen historical snapshot, not a live regeneration pipeline AGB needs. Copied `docs/mcp-multi-repo-rollout.md` + `docs/micro-ui-agent-builder-future-ai-stack-prd.md` from MUI into this repo's `docs/`, each with a short provenance header (kept the body verbatim, since sibling repos `tabletop-studio`/`board-game-sim-ai` reference this exact content).
- **Step 5 (verification done; found a live production incident, unrelated to code):** full gate green — `pnpm -r typecheck`/`lint` (0 errors, same 4 pre-existing warnings), `pnpm run test` (sdk 29 + studio 30), `pnpm run build` (15 routes), `cd backend && uv run pytest -q` (232). The "final real Vercel deploy check" surfaced that **production is currently down**: `GET /api/health` (and every other `/api/*` route) returns `FUNCTION_INVOCATION_FAILED`. Root cause (via `get_runtime_errors`): FastAPI's `bootstrap()` startup hook (`backend/app/main.py:73`) calls `storage.get_graph()` to seed the demo graph, which 403s against `https://z262bpbfgnmu1bb2.private.blob.vercel-storage.com/...` — the configured Vercel Blob store now requires a different credential than whatever `BLOB_READ_WRITE_TOKEN` is currently set to, or the store's access level changed. First occurrence recorded 2026-09-10, i.e. **predates this session's Phase 6/7/8 work entirely** (all frontend-only, no `storage.py`/`vercel_blob.py` changes) — not a regression introduced here. This is a dashboard-level credential/storage-config issue, not a code bug, and per this repo's own `AGENTS.md` ("Must not: run `vercel env` to set production secrets from agent sessions — operator sets dashboard vars"), fixing it is out of scope for an agent session. Flagging here rather than silently calling the gate green: **the app the whole studio-consolidation program built is not currently reachable in production**, independent of anything in this phase.
- **Step 6: blocked on the above, and still gated on explicit user confirmation regardless.** Archiving `micro-ui-agent-builder` is irreversible and affects a repo outside this one — not attempted.

### Phase 7 — UX parity and polish (post-cutover review)

A user review comparing the merged studio against both MUI's original design intent and the old (deleted) `apps/playground` UX surfaced concrete gaps. Investigated against the actual code and old playground source (recovered via `git show 2341c56^:apps/playground/src/<path>`) before any changes landed. Two of six original complaints were non-issues and are **no-ops**: the floating glass-panel top HUD already existed and matched MUI's documented chrome intent, and "Add node"/"Run"/orientation controls already rendered full text labels, not icon-only.

1. **Node visuals.** `GraphNodeView.tsx` was a byte-identical verbatim port of playground's small (~130–150px) minimal cards, never upgraded to MUI's richer card language. Widened cards to ~256px (router diamond to 220px, enlarged proportionally), added base padding (previously most node types had none at all beyond shape-specific overrides), bumped icon size 12px→16px, and added a per-type ambient glow (`nodeTypeGlow()`, new in `graph-theme.ts`) built from the existing `nodeType` accent hex values rather than adopting MUI's Tailwind-class-based glow syntax — kept the inline-style token system the canvas already uses.
2. **Persistent layout panels.** The floating node palette and `RunPanel` defaulted to closed and had no responsive fallback or in-canvas graph switcher — a real, undocumented regression against the old playground's docked-by-default panels + `GraphLibrary` + `ShellDrawer` responsive system. Ported `useShellLayout.ts`/`shellLayout.ts` (breakpoint state machine) and `ShellDrawer.tsx` (restyled to studio's `.glass-panel`/`.ghost-border` Tailwind classes instead of playground's inline `surface.panel` styles) from playground. Palette/run panel now default open on desktop (computed once via a `window.innerWidth` check at mount, not tied to live resize) and collapse into slide-in drawers at compact/phone widths. Added a new minimal `GraphLibrary.tsx` — list-and-select only, no create/export/import (those stay on `/graphs`) — toggled via a new "Switch graph" HUD button, so switching graphs no longer requires leaving the canvas; reuses the existing `[graphId]`-keyed reload effects, so no special-case navigation logic was needed. Left the outer app shell's `isGraphCanvasRoute` sidebar-hiding untouched — that's a correct, deliberate 1:1 port of MUI's own behavior, not part of this regression.
3. **Skeleton/shimmer — bug fix, not a design gap.** `Skeleton.tsx`'s `.agb-skeleton` class has been used in three places (`RunPanel.tsx`, `ProviderModelPicker.tsx`, `FlowCanvas.tsx`) since the studio port, but its shimmer CSS was never copied from playground's `index.css` into `apps/studio/app/globals.css` — every usage rendered as an invisible unstyled box. Ported the `@keyframes agb-shimmer` + `prefers-reduced-motion` override, substituting the old hardcoded hex values for the existing `--surface-container`/`--surface-container-high` Synthesis Dark tokens. Confirmed compiled into the production CSS bundle (`grep` on `.next/static/chunks/*.css`), including the reduced-motion rule. Left the top-level list pages' spinner+"Loading studio data…" pattern untouched — that's a faithful match to MUI's own design, not a regression.
4. **Nav regroup.** Split the flat two-group `studioNavGroups` (`"Build"` = Graphs + all 6 anatomy items; `"Operate"` = Runs/Analytics) into three: `"Agent Graphs"` (Graphs only — the app's main value point), `"Agent Anatomy Management"` (Agents/Prompts/Tools/MCP/LLM Profiles/GenUI), `"Observability & Analytics"` (Runs/Analytics, renamed from "Operate"). This supersedes Phase 4c's originally-locked grouping decision. No change needed to the root `/` → `/graphs` redirect — already the landing page.
5. **Right-click context menu — new feature, not a restoration.** Confirmed with the user before building: no equivalent ever existed in AGB's playground, the merged studio, or MUI. New `NodeContextMenu.tsx`, modeled on `ConnectKindMenu.tsx`'s floating-menu-at-cursor pattern (same dismiss-on-outside-click/Escape behavior), generalized to a plain `{label, onClick, tone?}` action list so one component covers all three contexts. Wired new `onNodeContextMenu`/`onEdgeContextMenu`/`onPaneContextMenu` props through `FlowCanvas.tsx` (ReactFlow's native context-menu events, `preventDefault()`-ed to suppress the browser's own menu) up to `GraphEditor.tsx`. Node right-click selects the node then offers Duplicate (new `duplicateNode()`, clones with a +40/+40 position offset) and Delete (reuses the existing `deleteSelection()` unmodified, since right-click now sets selection state first); edge right-click selects then offers Delete; empty-canvas right-click lists all 12 addable node types at the exact click position (`addNode()` gained an optional `position` parameter, defaulting to its prior random-offset behavior for `NodePalette`'s existing call sites, which pass no position) — this also fixes the pre-existing random-placement UX for palette-added nodes' most common alternative path.
6. **No action.** Explicitly flagged so the original complaints aren't silently dropped: the HUD/toolbar and full-text button labels already matched what was asked for.

**Gate:** `pnpm -r typecheck`/`lint` (0 errors, same 4 pre-existing studio warnings), `pnpm run test` (sdk 24 + studio 30, unchanged count — no new test files added this phase, existing coverage stayed green), `pnpm run build` (sdk + studio, 15 routes), `cd backend && uv run pytest -q` (226, untouched — this phase is frontend-only).

### Phase 8 — Workbench promotion, command palette, and chat sessions

Unlike Phase 7, this phase wasn't a gap-closing review — the user set a product direction after seeing the merged studio: don't recreate LangGraph/LangSmith/Langfuse/n8n, be a native workbench where the graph engine, resource registries, and a chat scratchpad are all one hotkey/menu/right-click away without a navigation event. Two scope decisions were confirmed directly with the user, not derived from a plan-doc gap: (1) chat messages run as a **direct model scratchpad** — free-form chat straight to a chosen provider/model, bypassing the graph engine entirely, not bound to a node and not a multi-turn graph execution; (2) session persistence (save/reopen) is a **day-one requirement**, not a fast-follow.

1. **Part A — App-wide workbench.** `paletteOpen`/`runPanelOpen`/`libraryOpen` were local `useState` inside `GraphEditor.tsx`, with the docked-div-vs-`ShellDrawer` ternary hand-written three times and `useShellLayout()` called only there — no other route could open a drawer. New `WorkbenchProvider.tsx` hosts a single `useShellLayout()` instance and one `activePanel: WorkbenchPanelId | null` (replacing the three booleans), plus a global keydown handler over a static registry (`panels.ts`'s `WORKBENCH_PANELS`) so a panel's hotkey is part of its declaration, not bolted on later. New `WorkbenchDrawer.tsx` centralizes the docked-vs-drawer rendering, parameterized by panel id, and switched from `absolute` to `fixed` positioning specifically so it also works when mounted at the `studio-shell.tsx` level (where the nearest positioned ancestor doesn't fill the viewport the way `GraphEditor`'s full-bleed canvas root does). `GraphEditor.tsx`'s three panels were migrated to register with `useWorkbench()` instead of owning state; five new thin `ResourceBrowserPanel<T>` instances (Agents/Prompts/Tools/MCP/LLM Profiles — list-only, "Open full page" links out) are mounted once in `studio-shell.tsx`, reachable from every route.
2. **Part B — Command palette.** Wired the already-built, zero-call-site `command.tsx` (`cmdk`) into `CommandPalette.tsx`, bound to Cmd/Ctrl+K (guarded by the existing `isEditableKeyboardTarget` check). Lists every studio route (from `studioNavGroups`, exported for this purpose) plus every `scope: "global"` workbench panel. The plan's own text anticipated a "recent sessions" group here too, but deferred it — chat sessions didn't exist until Part C — so it landed in Part E instead (below), not this part.
3. **Part C — Backend multi-turn chat + `ChatSession`.** `ChatModel.generate()` (`backend/app/providers/base.py`) was single-turn; added an optional `history: list[dict[str, str]] | None = None` parameter, defaulting to `None` so `compute_llm`'s existing call site is unaffected, and fanned it out across all 6 adapters (the OpenAI-compatible ones splice `history` between the system and final user message; `google.py` maps it into Gemini's `contents` array with `assistant`→`model` role mapping; `stub.py` ignores it — it's an offline keyword-matcher with no real conversational state). New `ChatMessage`/`ChatSession` Pydantic models reuse the existing generic per-kind resource seam (`storage.py`'s `save_resource`/`get_resource`/...) — no schema migration, no new storage backend — registered as a 6th kind (`"chat_sessions"`) alongside prompts/tools/mcp_servers/agents/llm_profiles, getting free CRUD via `_register_resource_routes`. One bespoke route outside that generic loop, `POST /api/chat-sessions/{id}/messages` (not a CRUD operation): loads the session, appends the user message, resolves the provider adapter, calls `generate(history=...)`, appends the reply, persists, returns the updated session.
4. **Part D — SDK additions.** `chatMessageSchema`/`chatSessionSchema` (Zod, mirroring the Pydantic shapes) in `schemas.ts`; `ChatMessage`/`ChatSession` types via `z.infer<>` in `types.ts` (Phase 4f's established convention); `client.ts` gained `chatSessions: resourceClient<ChatSession>(...)` (free CRUD, same factory as every other kind) plus one bespoke `sendChatMessage(sessionId, content)` alongside it for the non-CRUD route.
5. **Part E — Chat UI.** Phase 4e explicitly declined to port MUI's AI Elements chat components because AGB's Run surface had no free-form multi-turn chat use case — the new scratchpad *is* that use case, so this is where they finally landed, but as a deliberately minimal subset rather than a verbatim port: MUI's `conversation.tsx`/`message.tsx`/`prompt-input.tsx`/`shimmer.tsx` depend on `use-stick-to-bottom`, `motion/react`, `nanoid`, and the `ai` package's `UIMessage`/`ChatStatus`/`FileUIPart` types — none of which this repo has, and the scratchpad's actual needs (auto-scroll on new messages, a chat bubble, a textarea+send composer, a loading indicator) don't warrant adding any of them. Wrote lean from-scratch equivalents in `apps/studio/components/ai-elements/` instead (plain ref-based auto-scroll, Tailwind-only styling, the existing `.agb-skeleton` shimmer reused for the loading state). `tool.tsx`/`reasoning.tsx`/`code-block.tsx` were skipped entirely, per the plan — no tool-calling or extended-thinking to render. `ChatPanel.tsx` (registered as the `"chat"` panel, `mod+shift+c` hotkey) is a session picker (`client.chatSessions.list/get/create`) plus the ported composer, reusing `RunPanel.tsx`'s existing `ProviderModelPicker` for new-session provider/model selection rather than building a second one. While wiring this up, closed the Part B "recent sessions" gap: `CommandPalette` now fetches the session list on open and lists the 5 most recent, each opening the chat panel pre-selected via a new optional `context` argument on `WorkbenchProvider.open()` (additive — every existing single-argument call site is unaffected). The docked chat panel's `WorkbenchDrawer` wrapper deliberately uses a definite `h-[70vh]` rather than `max-h-*`, avoiding from the outset the exact height-resolution ambiguity flagged in the interleaved run-panel scrolling bug fix below (a `max-h-*`-only ancestor gives an `h-full` descendant nothing definite to resolve against until content already overflows).

**Gate:** `pnpm -r typecheck`/`lint` clean (0 errors, same 4 pre-existing studio warnings — one new transient warning from an unused `eslint-disable` comment was caught and removed during Part E), `pnpm run test` (sdk 29 + studio 30 — SDK grew from 24 with the new `chatSessions`/`sendChatMessage` schema and client tests; no new studio test files, existing coverage stayed green), `pnpm run build` (sdk + studio, 15 routes), `cd backend && uv run pytest -q` (232, up from 226 — new `test_chat_sessions.py` plus per-adapter `history` coverage in the existing provider test files). No dedicated component tests were added for the new `ai-elements`/`ChatPanel`/`CommandPalette` "recent sessions" UI — consistent with the rest of the workbench (e.g. `ResourceBrowserPanel` also has none), a gap worth closing in a future pass rather than this phase.
