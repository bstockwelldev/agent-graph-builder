...
#### Transform surfaces (implemented 2026-09-25)

One engine (`backend/app/transforms.py`), three surfaces:

| Surface | Where | Binding |
| --- | --- | --- |
| Edge transform | `edge.transform`; applied in `ports.resolve_node_input` | inline, or `transform_id` |
| `transform` node | palette → Transform; `nodes.compute_transform` | inline config, or `transformId` |
| Transforms library | Resources → Transforms; `/api/transforms` (versioned) | referenced by both of the above |
...
Non-blocking codes: `CONTRACT_SCHEMA_UNKNOWN` (a source declares no schema where the target expects one, or a schema keyword outside `type`/`required`/`properties`/`items`/`enum` is present — compatibility can't be verified either way). Slice B's implementation surfaced one more: the default port catalog itself has real kind mismatches baked in (`tool`'s `structured-json` input fed by a router's `message` output; `tool`'s `tool-result` output feeding `output`'s `message` input) that every existing graph, including the demo graph, already relies on. `EDGE_CONTRACT_KIND_INCOMPATIBLE` is therefore only ever blocking when *both* the source and target ports are author-declared (`input_ports`/`output_ports` explicitly set) — a kind mismatch where either side is still catalog-inferred downgrades to the non-blocking `CONTRACT_KIND_INFERRED_MISMATCH`, consistent with this document's own "A graph with no explicit ports is normalized using node-type defaults and only warned when an inferred contract is too broad to verify" (Current state and migration constraints).

`NODE_INPUT_PORT_AMBIGUOUS_BINDING`'s "sibling" exemption is implemented via reachability, not literal adjacency: two converging edges are exempt when their sources are each exclusively reachable from a different branch of a common upstream router/branch node (computed by forward BFS from each of that router's outgoing edges), not only when they are the router's own immediate outgoing edges. This is required for the demo graph itself to validate clean — its `output_1` node's two incoming edges come from `tool_lookup` and `llm_answer`, both two hops downstream of `router_1`'s two branches, not literal siblings of `router_1`.
